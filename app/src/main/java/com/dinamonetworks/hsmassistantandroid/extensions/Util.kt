package com.dinamonetworks.hsmassistantandroid.extensions
...
import com.dinamonetworks.hsmassistantandroid.data.errorBody
...
fun loginWithPreviusCredentialsAndMakeRequest(fragment: Fragment, makeRequest: (updatedToken: String) -> Unit?, authErrorCallback: (message: String) -> Unit?) {
    ...
    val callback = object : Callback<ResponseBody1> {
        override fun onFailure(call: Call<ResponseBody1>?, t: Throwable?) {
            ...
            authErrorCallback("Serviço não iniciado")
        }
...
fun fieldsAreValid(context: Context?, mTextInputLayoutArray: Array<TextInputLayout>): Boolean {
    ...
    for(mTextInputlayout: TextInputLayout in mTextInputLayoutArray) {
        ...
        if(input.isEmpty()) {
            mTextInputlayout.error = context!!.getString(com.dinamonetworks.hsmassistantandroid.R.string.required_field)
            isValid = false
        }
    }
    return isValid
